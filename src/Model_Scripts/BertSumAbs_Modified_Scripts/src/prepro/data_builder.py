...
def greedy_selection(doc_sent_list, abstract_sent_list, summary_size):
    ...
    for s in range(summary_size):
        ...
        for i in range(len(sents)):
            ...
            #logger.info('ROUGE at %d for sents %s: %0.4f'%(s,' '.join([str(x) for x in c]),rouge_score))
    ...
    logger.info('%s, %0.2f'%(' '.join([str(x) for x in selected]),cur_max_rouge))
...
class BertData():
    def __init__(self, args):
        self.args = args
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased-DA',do_lower_case=True)
...
def format_to_lines(args):
    ...
    for corpus_type in ['valid', 'test', 'train']:
        temp = []
        for line in open(pjoin(args.map_path, 'mapping_' + corpus_type )):
            temp.append(line.strip()[:-4])
