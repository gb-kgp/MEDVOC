...
class Translator(object):
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            self.logger.info(f"Generated summary token ids:{' '.join([str(int(n)) for n in preds[b][0]])}")
            ...
            raw_src = self.vocab.convert_ids_to_tokens([int(t) for t in src[b]])[:512]
            ...
            if(self.args.block_trigram):
                ...
                if(cur_len>3):
                    for i in range(alive_seq.size(0)):
                        ...
                        words = self.vocab.convert_ids_to_tokens([int(w) for w in words])
